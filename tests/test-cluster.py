...
# Privide a list of VMs you want to reuse. VMs should have already microk8s installed.
...
class VM:
    ...
    def __init__(self, attach_vm=None):
        ...
        if path.exists('/snap/bin/multipass') and False:
            ...
            print('WARNING: The lxc backend is experimental')
...
class TestCluster(object):
    ...
    def setup_cluster(self):
        ...
        if not reuse_vms:
            ...
            for i in range(0, size):
                ...
                get_all = vm.run('/snap/bin/microk8s.kubectl get all -A -o wide')
                print(get_all)
                self.VM.append(VM())
        else:
    ...
    def test_cilium_in_nodes(self):
        """
        Test each node has a cilium pod.
        """
    ...
    def test_nodes_in_ha(self):
        """
        Test all nodes are seeing the database
        """
        ...
        print('Removed vm {}'.format(self.VM[0].vm_name))
        ...
        while True:
            ...
            if attempt <= 0:
                assert False
            time.sleep(5)
        print('Cilium on the nodes running')
        ...
        print('Wait for two ingress to appear')
        time.sleep(10)
        attempt = 100
        while True:
            ready_pods = 0
